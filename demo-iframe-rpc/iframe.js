...
async function initRpcClient() {
    try {
        ...
        server = newRpcClient('DemoServerModule'); // Name required for server-side methods
        // Test the connection
        const serverInfo = await server.getServerInfo();
        log('Server info received successfully');
        showResult('Server Info', serverInfo);
        const parentClient = newRpcClient(); // Name not required for parent-side methods
...
export { initializeIframeClient, testServerMethod, clearLog };
