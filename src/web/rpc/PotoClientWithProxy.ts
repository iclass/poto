...
/**
 * Configuration for endpoint domains
 */
export interface EndpointConfig {
  /** Base URL for the main API endpoint */
  baseUrl: string;
...
/**
 * PotoClientWithProxy extends PotoClient and adds built-in web relay/proxy features.
 * It can be used as a drop-in replacement for PotoClient, but with integrated message listening and handler registration.
 * Enhanced with flexible endpoint domain configuration.
 * 
 * @example
 * ```typescript
 * // Simple usage with string (backward compatible)
 * const client = new PotoClientWithProxy('/api');
 * 
 * // Advanced usage with domain-specific endpoints
 * const client = new PotoClientWithProxy({
 ...
 * });
 * 
 * // Use domain-specific proxies
 * const aiAssistant = client.getProxy<AiAssistant>('AiAssistant', 'aiAssistant');
 * const ttsService = client.getProxy<TTSService>('TTSService', 'tts');
 * 
 * // Test endpoints
 * const health = await client.getHealthStatus();
 * console.log('AI Assistant health:', health.services.aiAssistant?.status);
 * ```
 */
export class PotoClientWithProxy extends PotoClient {
  ...
  // Only one endpoint configuration
  ...
  constructor(
    ...
    const config = typeof endpointConfig === 'string' 
      ? { baseUrl: endpointConfig } 
      : endpointConfig;
    
    super(config.baseUrl, storage);
   ...
   */
  updateEndpointConfig(newConfig: Partial<EndpointConfig>): void {
    this.endpointConfig = { ...this.endpointConfig, ...newConfig };
  }

  /**
   * getProxy method (no serviceName)
   */
  getProxy<T extends object>(modulePrefix: string): T {
    return super.getProxy<T>(modulePrefix);
  }
  ...
  async testEndpoint(): Promise<boolean> {
    try {
      const response = await fetch(`${this.endpointConfig.baseUrl}/health`, { 
        method: 'GET',
        signal: AbortSignal.timeout(this.endpointConfig.timeout || 5000)
      });
      return response.ok;
    } catch (error) {
      console.warn('[PotoClientWithProxy] Main endpoint test failed:', error);
      return false;
    }
  }

  /**
   * Get health status of the main endpoint
   */
  async getHealthStatus(): Promise<{
    main: { url: string; status: 'healthy' | 'unhealthy' | 'unknown' };
  }> {
    const health = {
      main: { url: this.endpointConfig.baseUrl, status: 'unknown' as 'healthy' | 'unhealthy' | 'unknown' }
    };
    try {
      const response = await fetch(`${this.endpointConfig.baseUrl}/health`, { 
        method: 'GET',
      ...
      health.main.status = response.ok ? 'healthy' : 'unhealthy';
    } catch (error) {
      health.main.status = 'unhealthy';
    }
    return health;
  }
  ...
  private async handleRpcRequestCascaded(request: RpcRequest, event: MessageEvent): Promise<any> {
    ...
    if (iframeId) {
      ...
      console.error('[PotoClientWithProxy] SHOULD NOT HAPPEN: No iframeId identified for event, method:', method);
    }
    ...
    if (request.moduleName) {
      ...
      if (!serverProxy) {
        serverProxy = this.getProxy(request.moduleName);
    ...
    console.error(`[PotoClientWithProxy] RPC method not found in any handler: ${method}`);
